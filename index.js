...
async function fetchBotReply(outline) {
  ...
  movieBossText.innerText = completion.choices[0].text.trim();
}
...
async function fetchSynopsis(outline) {
  ...
  const synopsis = completion.choices[0].text.trim();
...
async function fetchTitle(synopsis) {
  ...
  const title = completion.choices[0].text.trim();
...
async function fetchImagePromt(title, synopsis) {
  ...
  fetchImageUrl(completion.choices[0].text.trim());
}
...
async function fetchImageUrl(imagePrompt) {
  const completion = await openai.createImage({
    ...
    n: 1,
    size: "256x256",
    completion_format: "url",
  });
  document.getElementById(
    "output-img-container"
  ).innerHTML = `<img src="data:image/png;base64,${completion.data[0].url}">`;
